...
const ListPackage = () => {
  ...
  const handleTrackPackage = async (code) => {
    ...
    const res = await api.trackingCode(code, token)
  ...
  const handleSearchPackage = async (e) => {
    ...
    if (searchType.trim() != '' && searchPackage.trim() != '') {
      try {
        const res = await api.searchPackage(searchType, searchPackage, token)
        setData(res)
      } catch (error) {
        return null
      }
    } else {
      const res = await api.listPackage(page, token)
      setData(res.data)
    }
  ...
  const handleEditPackage = async (code) => {
    ...
    setEditPackage(code)
    onOpen()
    const res = await api.getPackage(code, token)
      ...
      >
        ...
        >
          ...
          <Flex
            ...
            w="100%"
          >
            <Flex w="100%" overflowX={data.data == '' ? 'hidden' : 'scroll'}>
                    ...
                    {data.data.map((code, i) => (
                      <Tr key={i}>
                        ...
                        <Td display="flex" flexDir="column">
                          ...
                          <Tooltip label="Disponível em breve">
                            <Button onClick={() => handleDeletePackage(code.id)} colorScheme="whatsapp" m="2px 0" leftIcon={<FaWhatsapp />} >
                              Enviar Mensagem
                            </Button>
                          </Tooltip>
                        </Td>
